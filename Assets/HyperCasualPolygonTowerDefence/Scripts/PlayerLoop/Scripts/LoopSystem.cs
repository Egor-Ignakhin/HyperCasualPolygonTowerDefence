...
namespace HyperCasualPolygonTowerDefence.Scripts.PlayerLoop.Scripts
{
    internal class LoopSystem<TUpdateGroup> : ILoopSystem
    {
        ...
        private int count;
        private readonly List<int> updatablesToRemove = new();

        private void OnUpdate()
        {
            foreach (var elementIndex in updatablesToRemove) RemoveAtReal(elementIndex);
            updatablesToRemove.Clear();
            for (var i = 0; i < count; i++) updatables[i].Updatable();
        }

        public IDisposable Start(Action updatable)
        {
            if (count == updatables.Length) Array.Resize(ref updatables, (int)(count * 1.5f));
        ...
        {
            private readonly LoopSystem<TUpdateGroup> loopSystem;
            internal int Index;
